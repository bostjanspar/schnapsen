...
describe('StateMachine', () => {
  ...
  describe('State Registration', () => {
    test('should add states successfully', () => {
      ...
      //@ts-ignore
      expect(stateMachine.states.has(StateEnum.TEST_STATE_A)).toBe(true);
    });

    test('should prevent duplicate state IDs', () => {
      stateMachine.addState(mockStateA);
      const anotherStateA = new TestStateA(StateEnum.TEST_STATE_A);
      expect(() => stateMachine.addState(anotherStateA)).toThrow('State with ID TEST_STATE_A already exists in this state machine.');
    });
  ...
  describe('State Transitions', () => {
    ...
    test('should transition between valid states', () => {
      //@ts-ignore
      expect(stateMachine.currentState.id).toBe(StateEnum.TEST_STATE_A);
      stateMachine.transition(StateEnum.TEST_STATE_B);
      //@ts-ignore
      expect(stateMachine.currentState.id).toBe(StateEnum.TEST_STATE_B);
    });
    ...
    test('should reject invalid transitions', () => {
      ...
      const result = stateMachine.transition(StateEnum.TEST_STATE_C);
      expect(result).toBe(false);
      expect(consoleWarnSpy).toHaveBeenCalledWith('Target state TEST_STATE_C not found in this state machine.');
  ...
  describe('Event Processing', () => {
    ...
    test('should pass events to current active state', () => {
      ...
      stateMachine.onEvent(EventEnum.CONSUME_EVENT);
      expect(mockStateA.eventCallCount).toBe(1);
      expect(mockStateA.lastEvent).toBe(EventEnum.CONSUME_EVENT);
    });

    test('should return event processing result', () => {
      const consumed = stateMachine.onEvent(EventEnum.CONSUME_EVENT);
      expect(consumed).toBe(true);
      const notConsumed = stateMachine.onEvent(EventEnum.DO_NOT_CONSUME_EVENT);
    ...
    test('should not process events if no active state', () => {
        ...
        const result = sm.onEvent(EventEnum.CONSUME_EVENT);
        expect(result).toBe(false);
        expect(mockStateA.eventCallCount).toBe(0);
    });
  });
